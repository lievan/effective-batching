...
def generate(inference, model):
    ...
    stacked = torch.stack(
        [torch.tensor(encoding, dtype=torch.long, device=model.device)]
    )
    if LOGGING:
        print("INFERENCE LOGS: Making inference (no batching): ", end="")
...
def static_batch_generate(batch, model):
    ...
    stacked = torch.stack(
        [
            torch.tensor(start_ids, dtype=torch.long, device=model.device)
            for start_ids in encoded_prompts
        ]
    )
    ...
    if LOGGING:
        print("INFERENCE LOGS: Making inference (static batching): ")
    for i in range(max_desired_tokens):
        ...
        stacked = torch.cat((stacked, idx_next), dim=1)
        if LOGGING and i < 10:
            print("[{}]".format("*" * len(stacked)))
        elif LOGGING and i == 11:
            print(".... [TRUNCATED]")
    for res, inference in zip(stacked, batch):
        results.append(
            (
                model.decode(
                    res.tolist()[: (len(inference.data) + inference.num_tokens)]
                ),
                inference,
            )
        )
...
def static_batch_generate_v2(batch, model):
    ...
    prompts_and_desired_tokens.sort(
    ...
    for tokens_generated in range(max_desired_tokens + 1):
        if prompts_and_desired_tokens[-1][1] == tokens_generated:
            ...
            results.append(
                (stacked[prompt_id], batch[prompt_id])
            )  # append completion to results array
            stacked = torch.cat(
                (stacked[:prompt_id], stacked[prompt_id + 1 :])
            )  # remove finished prompt from batch
...
def dynamic_batch_generate(next_batch, model):
    ...
    if LOGGING:
        print(
            "INFERENCE LOGS: Making inference (dynamic batching) | in progress {}, finished {}".format(
                len(in_progress), len(finished)
            )
        )
